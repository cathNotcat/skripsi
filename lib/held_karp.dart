...
class DistanceDuration {
  ...
  DistanceDuration(this.distance, this.duration);
}
...
class HeldKarp {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data.containsKey('features') &&
          ...
          double distanceKm = (segment['distance'] ?? 0) / 1000;
          double durationMin = (segment['duration'] ?? 0) / 60;
          print(
              'Distance: $startLong → $endLong = $distanceKm km | $durationMin min');
          return DistanceDuration(double.parse(distanceKm.toStringAsFixed(2)),
              double.parse(durationMin.toStringAsFixed(2)));
          // return DistanceDuration(distanceKm, durationMin);
        } else {
          print('Failed to fetch ORS: ${response.statusCode}');
        }
      }
    } catch (e) {
      print('Error in getDistanceDuration: $e');
    }
    return DistanceDuration(double.infinity, double.infinity);
  }
  ...
  Future<Map<String, dynamic>> calculateWithHeldKarp(
      List<LatLng> points) async {
    // Future<List<LatLng>> calculateWithHeldKarp(List<LatLng> points) async {
    int n = points.length;

    // Set the matrix, default is 0
    List<List<DistanceDuration>> dist =
        List.generate(n, (_) => List.filled(n, DistanceDuration(0, 0)));
    ...
    for (int i = 0; i < n; i++) {
      for (int j = i + 1; j < n; j++) {
        ...
        var result = await getDistanceDuration(points[i].longitude,
        ...
        var reverse = await getDistanceDuration(points[j].longitude,
            points[j].latitude, points[i].longitude, points[i].latitude);
        dist[j][i] = reverse;
      }
    }
      ...
      for (int u = 0; u < n; u++) {
        ...
        for (int v = 0; v < n; v++) {
          ...
          if ((mask & (1 << v)) != 0 && u != v) {
            ...
            dp[mask][u] =
                min(dp[mask][u], dp[mask ^ (1 << u)][v] + dist[v][u].distance);
            print(
                "Updating dp[$mask][$u] = min(${dp[mask][u]}, ${dp[mask ^ (1 << u)][v]} + ${dist[v][u].distance})");
          }
        }
      }
    }
    ...
    for (int i = 1; i < n; i++) {
      ...
      double cost = dp[(1 << n) - 1][i] + dist[i][0].distance;
      // find the minimum
      if (cost < minCost) {
        minCost = cost;
    ...
    while (currentIndex != -1) {
      ...
      for (int j = 0; j < n; j++) {
        ...
        if ((mask & (1 << j)) != 0 &&
            (dp[mask][j] +
                        dist[j][currentIndex].distance -
                        dp[mask | (1 << currentIndex)][currentIndex])
                    .abs() <
                1e-6) {
          totalDuration += dist[j][currentIndex].duration;
          // currentIndex = j;
          prevIndex = j;
          break;
        }
      ...
      if (prevIndex == -1) {
        print("Failed to reconstruct path, to prevent infinite loop.");
    ...
    // add duration form last city to return to start
    totalDuration += dist[lastIndex][0].duration;
    path.add(points[0]);

    print('min cost: $minCost km');
    print('estimated duration: $totalDuration minutes');
    return {
      'minCost': minCost,
